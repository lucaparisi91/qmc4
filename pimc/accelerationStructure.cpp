...
void listFortranStorage(
    ...
    int * cellIndexPerParticle,   int * subIndexPerParticle,
     allocator_t & alloc,
     int startParticle,int endParticle, int N, int & nTotParticles)
{
     ...
     int startParticle,int endParticle, int N)
{
    ...
    for(int i=startParticle;i<=endParticle;i++)
    {
        ...
        if ( cellIndexPerParticle[i]!=index )
        {
            ...
            if ( nParticlesPerCell[iCellOld] > 0 )
            {
                int j=(cells[iCellOld])[
                nParticlesPerCell[iCellOld] - 1
            ];

                std::swap( (cells[iCellOld])[iiCellOld],(cells[iCellOld])[
                nParticlesPerCell[iCellOld] - 1
            ] );
                subIndexPerParticle[j]=iiCellOld;
            }
...
void removeList(
    ...
    int** cells, int * nParticlesPerCell,int * nParticlesPerCellAllocated,
    int * cellIndexPerParticle,   int * subIndexPerParticle,
     int startParticle,int endParticle, int & nTotParticles)
{
    ...
    for(int i=startParticle;i<=endParticle;i++)
    {
            ...
            int iiCell=subIndexPerParticle[i];

            int j= (cells[iCell])[
                nParticlesPerCell[iiCell] - 1
            ] ;
            if (nParticlesPerCell[iCell] > 0 )
            {
                std::swap( (cells[iCell])[iiCell],(cells[iCell])[
                nParticlesPerCell[iCell] - 1 ] );
                subIndexPerParticle[j]=iiCell;
            }
...
void simpleCellNeighbourList::add(const double * positions, size_t N , int iStart, int iEnd )
{
    assert( iStart>=0 );
    assert( iEnd<N );
    ...
    if ( dimensions==3)
    {
        ...
        intAllocator,iStart,iEnd,N,nTotParticles);
...
void simpleCellNeighbourList::update(const double * positions, size_t N, int iStart, int iEnd)
{
...
void simpleCellNeighbourList::remove( int iStart, int iEnd)
{

        removeList( nCells.data(),cells.data() , nParticlesPerCell.data(),nParticlesPerCellAllocated.data() ,cellIndexPerParticle,subIndexPerParticle,iStart,iEnd, nTotParticles);

}


simpleCellNeighbourList::simpleCellNeighbourList(std::array<int,DIMENSIONS >  nCells_, std::array<Real,DIMENSIONS> lBox_, neighbourListPolicy policy_  ,  int nParticleStorageInitial):
                                   ...
                                   std::array<int,3> mIndexCell={iii,jjj,kkk};
                                    int cellIndexNeighbour =indexFortranStorage<3>(mIndexCell.data(),nCells.data());
                                                ...
                                                ) 
                                            {
                                                ...
                                                for(int iiNeighbour=0;iiNeighbour<iNeighbour;iiNeighbour++)
                                                {
                                                    if (indexCellNeighbours[cellIndex][iiNeighbour] == cellIndexNeighbour ) found=true;
                                                }

                                                if (not found)
                                                {
                                                    // store the neighbours
                                                indexCellNeighbours[cellIndex][iNeighbour]=cellIndexNeighbour; 
...
namespace pimc
{
    void cellNeighbourList::setGeometry(const geometry_t & geo)
    {
        for(int d=0;d< DIMENSIONS ; d++)
        {
            _lBox[d]=geo.getLBox(d);
        }
    }

    void cellNeighbourList::setNCells( const std::array<int,DIMENSIONS > & cells)
    {
        _nCells=cells;
    }

    void cellNeighbourList::build()
    {
        for(int t=0;t<_nBeads + 1 ; t++)
        {
            _timeSliceLists.push_back( 
                std::make_unique<acceleration::simpleCellNeighbourList>(_nCells,_lBox)
    ...
    void cellNeighbourList::add( const cellNeighbourList::data_t & data, const std::array<int,2> & timeRange, const std::array<int,2> & particleRange)
    {
        ...
        {
            _timeSliceLists[t]->add( data.data() + N*DIMENSIONS*t , N, particleRange[0],particleRange[1] );
        }
    }

    void cellNeighbourList::remove(  const std::array<int,2> & timeRange, const std::array<int,2> & particleRange)
    {
        
        
        for(int t=timeRange[0];t<=timeRange[1];t++)
        {
            _timeSliceLists[t]->remove( particleRange[0],particleRange[1] );
        }
    }


    void cellNeighbourList::update( const cellNeighbourList::data_t & data, const std::array<int,2> & timeRange, const std::array<int,2> & particleRange)
    {
        const auto & dimensions = data.dimensions();
        size_t N=dimensions[0]*dimensions[1];

        for(int t=timeRange[0];t<=timeRange[1];t++)
        {
            _timeSliceLists[t]->update( data.data() + N*t , N, particleRange[0],particleRange[1] );
        }
    }
