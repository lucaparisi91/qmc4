...
bool swapMove::attemptMove(configurations_t & confs , firstOrderAction & S,randomGenerator_t & randG )
{
    ...
    else if ( confs.getEnsamble() == ensamble_t::grandCanonical or confs.getEnsamble() == ensamble_t::semiGrandCanonical )
    {
...
bool closeMove::attemptSemiGrandCanonicalMove(configurations_t & confs , firstOrderAction & S,randomGenerator_t & randG )
{
    ...
    deltaS-=sPot.evaluate(confs,timeRangeB,iChainTailB);
    deltaS-=sPot.evaluate(confs,timeRangeB2,iChainTailBNext);
...
bool advanceHead::attemptMove(configurations_t & confs , firstOrderAction & S,randomGenerator_t & randG)
{
     if ( confs.getEnsamble() == ensamble_t::canonical)
    {
        throw std::runtime_error("AdvanceHead not allowed in a grand canonical simulation");
    }
    else if ( confs.getEnsamble() == ensamble_t::grandCanonical)
    {
        return attemptGrandCanonicalMove(confs,S,randG);
...
bool advanceHead::attemptGrandCanonicalMove(configurations_t & confs , firstOrderAction & S,randomGenerator_t & randG)
{
    Real timeStep = S.getTimeStep();
...
bool advanceHead::attemptSemiGrandCanonicalMove(configurations_t & confs , firstOrderAction & S,randomGenerator_t & randG)
{
    Real timeStep = S.getTimeStep();
    ...
    int iChainA=headsA[std::floor(uniformRealNumber(randG) * headsA.size() )];
    int iChainB=tailsB[std::floor(uniformRealNumber(randG) * tailsB.size() )];

    int l= sampleLength(randG);
    int M = confs.nBeads();

    
    int iChainBNext = confs.getChain(iChainB).next;
   

    int tHeadA=confs.getChain(iChainA).head;
    int tTailB=confs.getChain(iChainB).tail;

    assert(tHeadA == tTailB + 1 );

    // checks wether there are enough beads in B to be canceled otherwise abort move

    int tTailBNew=-100;
    if ( confs.getChain(iChainB).hasHead() )
    {
        int tBHead=confs.getChain(iChainB).head;
        tTailBNew = tTailB + l ;

        if (tTailB + l + 1 >= tBHead )
        {
            return false;
        }
    }
    ...
    {
        iChainBNext=confs.getChain(iChainB).next;
        int tBPrevHead=confs.getChain(iChainB).head;

        if (tHeadA + l > M)
        {
            tTailBNew = tTailB + l - M ;
        
            if ( tTailB + 1 + l  -  M >= tBPrevHead )
            {
                return false;
            }
    ...
    assert( (tTailBNew >= -1) and (tTailBNew < M ));


     if (tHeadA + l > M)
     {
         assert(iChainBNext >=0 );
        assert(tTailBNew < confs.getChain(iChainBNext).head );
     }
    ...
    std::array<int,2> timeRangeA={tHeadA  , std::min(tHeadA + l,M) - 1 };
    std::array<int,2> timeRangeA2={0  , tHeadA + l - M  - 1 };

    std::array<int,2> timeRangeB={tTailB + 1  , std::min(tTailB + 1 + l,M) - 1 };
    std::array<int,2> timeRangeB2={0  , tTailB  + l - M  };
    ...
    deltaS-=sPot.evaluate(confs,timeRangeB,iChainB);
    deltaS-=sPot.evaluate(confs,timeRangeB2,iChainBNext );
    
    // performs levy reconstruction on l beads
    _levy.setReconstructorBoundaries(chainBoundary::fixed,chainBoundary::free);
    _levy.apply(confs,{tHeadA,tHeadA+l},iChainA,S,randG);

    confs.setHead(iChainA,std::min(tHeadA+l,M));
    confs.setTail(iChainB,std::min(tTailB +l,M - 1 ));
    ...
    {
        // creates a new chain
        iChainANew=confs.pushChain(getSet());
        confs.setHeadTail(iChainANew,timeRangeA2[1] + 1,-1);
        confs.copyData( { M , tHeadA + l } , iChainA, 0,iChainANew );   

        // creates tail on the next chain
        confs.setTail(iChainBNext, tTailBNew );
    ...
    if (tHeadA + l > M)
    {
        deltaS+=sPot.evaluate(confs,timeRangeA2,iChainANew);
    }

    Real deltamu = confs.getChemicalPotential(setA) - confs.getChemicalPotential(setB) ;
    ...
    if ( accept)
    {
        if (tHeadA + l > M)
        {
            confs.join(iChainA,iChainANew);
            confs.removeChain(iChainB);
    ...
    {
        confs.setHead(iChainA,tHeadA);
        confs.setTail(iChainB,tTailB);
            
        if (tHeadA + l > M)
        {
            confs.setTail(iChainBNext,-1);
            confs.join(iChainB,iChainBNext);

            confs.removeChain(iChainANew);
        }
    }
...
bool recedeHead::attemptGrandCanonicalMove(configurations_t & confs , firstOrderAction & S,randomGenerator_t & randG)
{
    Real timeStep = S.getTimeStep();
    const auto & heads = confs.getGroups()[getSet()].heads;
...
bool recedeHead::attemptSemiGrandCanonicalMove(configurations_t & confs , firstOrderAction & S,randomGenerator_t & randG)
{
    ...
    // do not accept proposal which would destroy the chain
    int iChainPrevA=-1;

    if (confs.getChain(iChainA).hasTail() )
    {
        int tTailA=confs.getChain(iChainA).tail;
        
        if (tHeadA - l <= tTailA + 1)
        {
            return false;
        }
    }
    ...
    {
        iChainPrevA=confs.getChain(iChainA).prev;
        int tTailA=confs.getChain(iChainPrevA).tail;

        if (tHeadA - l + M <= tTailA + 1)
        {
            return false;
        }
    }
    ...
    std::array<int,2> timeRange={ std::max(tHeadA - l,0 ) , tHeadA - 1  };
    std::array<int,2> timeRange2={ tHeadA - l + M  , M - 1};
    ...
    deltaS-=sPot.evaluate(confs,timeRange,iChainA);

    if (tHeadA - l < 0 )
    {
        deltaS-=sPot.evaluate(confs,timeRange2,iChainPrevA);
    }

    confs.setHead(iChainA,std::max(tHeadA-l,0));
    if (tHeadA - l < 0 )
    {
        confs.setHead(iChainPrevA,timeRange2[0]  );
    }


    int iChainNewBTail = iChainB;
    confs.setTail(iChainB,std::max(tTailB-l,-1));
    ...
    {
          // creates a new chain
        iChainNewBTail=confs.pushChain(setB);
        confs.setHeadTail(iChainNewBTail, M , timeRange2[0] - 1);
        confs.join(iChainNewBTail,iChainB);
        for (int d=0;d<getDimensions(); d++)
        {
            data(iChainNewBTail,d,timeRange2[0]+l)=data(iChainB,d,timeRange[1] + 1 ); 
        }
        _levy.apply(confs,{timeRange2[0],timeRange2[0]+l },iChainNewBTail,S,randG);
        confs.copyData( { M , timeRange2[0] + l } , iChainNewBTail , 0,iChainB );   

    }
    else
    {
        _levy.apply(confs,{timeRange[0],timeRange[1] + 1 } , iChainB, S, randG );
    }


    deltaS+=sPot.evaluate(confs,timeRange,iChainB);
     if (tHeadA - l < 0)
     {
        deltaS+=sPot.evaluate(confs,timeRange2,iChainNewBTail);     
     }



    auto propRatio = -deltaS - ( confs.getChemicalPotential( setA ) - confs.getChemicalPotential(setB)  )*l*timeStep;
    ...
    else
    {
        ...
        {
            confs.setHead(iChainA,tHeadA);
            confs.setTail(iChainB,timeRange[1] );
        }
        else
        {
            confs.setHead(iChainA,tHeadA);
            confs.setHead(iChainPrevA,M);
            confs.join(iChainPrevA,iChainA);

            confs.setTail(iChainB,timeRange[1] );
            confs.removeChain(iChainNewBTail);
